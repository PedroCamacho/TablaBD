...
DROP PROCEDURE IF EXISTS `Superpoblado`; 
CREATE DEFINER=`root`@`localhost` PROCEDURE `Superpoblado`(IN `pobla` INT) NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER SELECT * FROM world.city WHERE Population >= pobla order by 2;

DROP TABLE IF EXISTS `users`;
CREATE TABLE `users` (
  `usr` char(100)  PRIMARY KEY, 
